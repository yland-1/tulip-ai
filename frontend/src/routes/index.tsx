...
function Brand({ compact = false }: { compact?: boolean }) {
  return (
    <div className="flex items-center gap-2.5" aria-label="TulipAI">
      <span className="flex size-9 items-center justify-center rounded-xl bg-zinc-100 text-zinc-900 shadow-sm">
        <Flower2 className="size-5" strokeWidth={1.8} aria-hidden="true" />
      </span>
      <span
        className={`${compact ? "text-lg" : "text-xl"} font-display font-semibold text-zinc-900 tracking-tight`}
      >
        Tulip<span className="text-zinc-500">AI</span>
      </span>
    </div>
  );
}
                  ...
                  >
                    ...
                    {message.clarification && (
                      <div className="mt-4 flex flex-wrap gap-3">
                        <Button
                          variant="outline"
                          className="flex items-center gap-2 border-zinc-200 bg-white hover:bg-zinc-100 text-zinc-900"
